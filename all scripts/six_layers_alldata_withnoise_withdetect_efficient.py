...
class BirdDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x_spec = spectrogram_transform(x)
        if self.use_mel:
            x_spec = mel_spectrogram_transform(x_spec)
        x_spec = db_scaler(x_spec)
        x_spec = resize(x_spec)
        prob_birdcall = detection_model(x_spec.unsqueeze(0))[0][1].item()
        
        label = None
        if prob_birdcall >= 0.5:
            label = self.labels[index]
        else:
            label = torch.zeros(len(species))
