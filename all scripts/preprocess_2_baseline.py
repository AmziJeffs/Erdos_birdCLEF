...
for epoch_num, epoch in enumerate(tqdm(range(NUM_EPOCHS))):
    ...
    if REPORT_VALIDATION_LOSS_PER_EPOCH == True:
        ...
        # Early stopping
        if validation_losses[epoch_num] < best_validation_loss:
            best_validation_loss = validation_losses[epoch_num]
            early_stopping_counter = 0
            # Save the best model
            torch.save(model.state_dict(), f"{CHECKPOINT_DIR}{MODEL_NAME}/best_model.pt")
        else:
        ...
        scheduler.step(validation_losses[epoch_num])
